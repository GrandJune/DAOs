...
def func(m=None, n=None, group_size=None, lr=None, turnover_rate=None,
    ...
    for period in range(search_loop):
        ...
        # Turnover
        if turnover_rate != 0:
            # individual turnover; without manager turnover
            for team in hierarchy.teams:
                for individual in team.individuals:
...
if __name__ == '__main__':
    ...
    time.sleep(delay)
    index = 1
    performance_file_name = "hierarchy_performance_across_only_individual_turnover_{0}".format(index)
    while os.path.exists(performance_file_name):
        index += 1
        performance_file_name = "hierarchy_performance_across_only_individual_turnover_{0}".format(index)

    with open("hierarchy_performance_across_only_individual_turnover_{0}".format(index), 'wb') as out_file:
        pickle.dump(performance_across_para, out_file)
    with open("hierarchy_diversity_across_only_individual_turnover_{0}".format(index), 'wb') as out_file:
        pickle.dump(diversity_across_para, out_file)
    with open("hierarchy_variance_across_only_individual_turnover_{0}".format(index), 'wb') as out_file:
        pickle.dump(variance_across_para, out_file)
