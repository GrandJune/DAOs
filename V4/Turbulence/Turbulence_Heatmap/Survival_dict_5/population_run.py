...
def func(m=None, n=None, group_size=None, lr=None, turbulence_freq=None,
    ...
    dao_percentage_list, hierarchy_percentage_list, autonomy_percentage_list = [1/3], [1/3], [1/3]
            ...
            if k > 0:
                ...
                for choice in choices:
                    if choice == "autonomy":
                        ...
                        hierarchy_list.append(Hierarchy(m=m, n=n, reality=reality, lr=lr, group_size=group_size))
            all_num = len(autonomy_list) + len(dao_list) + len(hierarchy_list)
            dao_percentage_list.append(len(dao_list) / all_num)
            hierarchy_percentage_list.append(len(hierarchy_list) / all_num)
            autonomy_percentage_list.append(len(autonomy_list) / all_num)
...
if __name__ == '__main__':
    ...
    for _ in range(hyper_repeat):
        ...
        autonomy_percentage_hyper += [result[2] for result in results]

    dao_percentage = np.mean(dao_percentage_hyper, axis=0)
    hierarchy_percentage = np.mean(hierarchy_percentage_hyper, axis=0)
    autonomy_percentage = np.mean(autonomy_percentage_hyper, axis=0)
    ...
    print("Turbulence Rate=0.2; Frequency=100, T=1001", time.strftime("%Y-%m-%d %H:%M:%S", time.gmtime(time.time())))  # Complete time
