...
def func(m=None, n=None, group_size=None, lr=None, initialization_bar=None, search_loop=None,
         loop=None, return_dict=None, sema=None):
    np.random.seed(None)
    reality = Reality(m=m)
    hierarchy = Hierarchy(m=m, n=n, reality=reality, lr=lr, group_size=group_size, initialization=initialization_bar)
    ...
    for initialization_bar in initialization_bar_list:
        ...
        for loop in range(repetition):
            ...
            p = mp.Process(target=func,
                           args=(m, n, group_size, lr, initialization_bar, search_loop, loop, return_dict, sema))
            jobs.append(p)
            p.start()
        for proc in jobs:
            proc.join()
