...
# @Time     : 6/5/2026
# @Author   : Junyi
# @FileName: hybrid_turbulence_run.py
# @Software  : PyCharm
...
def func(m=None, n=None, group_size=None, lr=None, threshold_ratio=None,
         ...
         alpha=3, p1=0.1, p2=0.9, token=False):
...
if __name__ == '__main__':
    ...
    turbulence_freq = 100          # T_tb = 100
    turbulence_intensity = 0.20    # I_tb = 0.20
    ...
    for recentralization in recentralization_list:
        ...
        for hyper_loop in range(hyper_iteration):
            ...
            for loop in range(repetition):
                ...
                p = mp.Process(
                    target=func,
                    args=(m, n, group_size, lr, threshold_ratio,
                          turbulence_freq, turbulence_intensity, search_loop,
    ...
    with open("hybrid_turbulence_variance", 'wb') as out_file:
        pickle.dump(variance_recentralization, out_file)
    with open("hybrid_turbulence_cv", 'wb') as out_file:
        pickle.dump(cv_recentralization, out_file)
    with open("hybrid_turbulence_entropy", 'wb') as out_file:
        pickle.dump(entropy_recentralization, out_file)
    with open("hybrid_turbulence_antagonism", 'wb') as out_file:
        pickle.dump(antagonism_recentralization, out_file)
    with open("hybrid_turbulence_consensus_performance", 'wb') as out_file:
        pickle.dump(consensus_recentralization, out_file)
    with open("hybrid_turbulence_mode", 'wb') as out_file:
        pickle.dump(mode_recentralization, out_file)

    t1 = time.time()
    print(time.strftime("%H:%M:%S", time.gmtime(t1 - t0)))  # Duration
    print("Hybrid turbulence: T_tb=100; I_tb=0.20",
          time.strftime("%Y-%m-%d %H:%M:%S", time.gmtime(time.time())))
