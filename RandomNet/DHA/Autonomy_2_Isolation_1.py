...
def func(num_nodes=None, num_neighbors=None, autonomous_prob=None, loop=None, return_dict=None, sema=None):
    ...
    for node in nodes:
        ...
        first_suggested_nodes = random.sample(list(set(nodes) - {node}), num_neighbors)
        for neighbor in first_suggested_nodes:
            # Check if the neighbor also suggests the focal node
            second_suggested_nodes = random.sample(list(set(nodes) - {neighbor}), num_neighbors)
...
if __name__ == '__main__':
    ...
    with open("num_subnetwork_across_autonomy_1", 'wb') as out_file:
