...
class Individual:
    ...
    def local_search_slim(self, payoff_flag=None):
        success = 0
        ...
        if payoff_flag == "discrete":
            self.payoff = self.reality.get_payoff_discrete(belief=self.belief)
            next_payoff = self.reality.get_payoff_discrete(belief=next_belief)
        else:
            self.payoff = self.reality.get_payoff_bidirection(belief=self.belief)
            next_payoff = self.reality.get_payoff_bidirection(belief=next_belief)
    ...
    def local_search_hover(self, payoff_flag=None):
        success = 0
        while True:
...
if __name__ == '__main__':
    ...
    success = 0
    for _ in range(200):
        success += individual.local_search_slim()
