...
def func(m=None, n=None, lr=None, threshold_ratio=None, search_loop=None, loop=None, return_dict=None, sema=None):
    np.random.seed(None)
    reality = Reality(m=m,)
    ...
    for threshold_ratio in threshold_ratio_list:
        ...
        for loop in range(repetition):
            ...
            p = mp.Process(target=func,
                           args=(m, n, lr, threshold_ratio, search_loop, loop, return_dict, sema))
            jobs.append(p)
            p.start()
        for proc in jobs:
            proc.join()
